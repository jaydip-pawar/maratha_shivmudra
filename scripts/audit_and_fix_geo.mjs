...
// Known alias map for common town / locality / colloquial entries
const talukaAliases = {
  // Satara
  ...
  "कांदिवली": { en: "Borivali", mr: "बोरिवली" }, // Kandivali is under Borivali taluka
  "मालाड": { en: "Borivali", mr: "बोरिवली" },
  "मुंबई": { en: "Borivali", mr: "बोरिवली" }, // Fallback for suburban if resident in Malad/Borivali
  ...
  // Thane
  "डोंबिवली": { en: "Kalyan", mr: "कल्याण" }, // Dombivli is under Kalyan taluka
  "मीरारोड": { en: "Thane", mr: "ठाणे" }, // Mira-Bhayandar area
...
async function audit() {
  ...
  const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/members?pageSize=200`;
  const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  const data = await res.json();
  const docs = data.documents || [];
  const regDocs = docs.filter(d => d.fields?.is_registered?.booleanValue === true);

  console.log(`Auditing ${regDocs.length} registered member records...\n`);
  ...
  for (const d of regDocs) {
    ...
    const address = f.address?.stringValue || '';
    const city = f.city?.stringValue || '';
    const village = f.village?.stringValue || '';

    // Check special districts (Belagavi, Surat)
    let isSpecial = false;
    ...
    if (!isSpecial) {
      ...
      if (!canonicalDist) {
        console.error(`UNKNOWN DISTRICT: ${id} (${name}) dist: "${rawDist}"`);
        continue;
      }
      ...
      if (talukaAliases[lowerT] || talukaAliases[rawTaluka]) {
        ...
        // Match against district's talukas
    ...
    // Special case fix: Dombivali entered under Mumbai City -> belongs to Thane (Kalyan taluka)
    if (id === '8433898993' && rawTaluka.toLowerCase() === 'dombivali' && canonicalDist.en === 'Mumbai City') {
      canonicalDist = distMap.get('thane');
      canonicalTaluka = { en: 'Kalyan', mr: 'कल्याण' };
    }

    const needsFix = 
      f.district_en?.stringValue !== canonicalDist.en ||
      f.district_mr?.stringValue !== canonicalDist.mr ||
      f.district_code?.stringValue !== canonicalDist.code ||
      f.state?.stringValue !== canonicalState.en ||
      f.state_code?.stringValue !== canonicalState.code ||
      f.sub_district?.stringValue !== canonicalTaluka?.en ||
      f.sub_district_mr?.stringValue !== canonicalTaluka?.mr ||
    ...
    analysis.push({
      ...
      current: {
        ...
        district: rawDist,
        district_en: f.district_en?.stringValue,
        district_mr: f.district_mr?.stringValue,
        sub_district: f.sub_district?.stringValue,
        sub_district_mr: f.sub_district_mr?.stringValue,
        taluka: f.taluka?.stringValue,
        city,
        address
      },
      canonical: {
  ...
  const toFix = analysis.filter(a => a.needsFix);
  console.log(`Found ${toFix.length} members with inconsistencies (out of ${analysis.length} total)`);
  
  // Group by district to inspect
  const byDist = {};
  for (const item of toFix) {
    const d = item.canonical.district;
    if (!byDist[d]) byDist[d] = [];
    byDist[d].push(item);
  }

  for (const [d, items] of Object.entries(byDist)) {
    console.log(`\n=== District: ${d} (${items.length} records to standardize) ===`);
    for (const it of items) {
      console.log(`  - [${it.phone}] ${it.name}`);
      console.log(`    FROM: Taluka: "${it.current.sub_district}" (mr: "${it.current.sub_district_mr}"), Dist: "${it.current.district}"`);
      console.log(`    TO:   Taluka: "${it.canonical.sub_district}" (${it.canonical.sub_district_mr}), Dist: "${it.canonical.district}" (${it.canonical.district_mr})`);
    }
  }

  return { analysis, toFix };
}
